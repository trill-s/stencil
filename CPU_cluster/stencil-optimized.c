...
void exchange_boundary_y(ptr_t a0,const dist_grid_info_t *grid_info)
{
    ...
    ptr_t tmp_send1= (ptr_t)malloc(sizeof(data_t)*(nx+2)*YLAP*nz);
    ptr_t tmp_send2= (ptr_t)malloc(sizeof(data_t)*(nx+2)*YLAP*nz);
    ptr_t tmp_recv1= (ptr_t)malloc(sizeof(data_t)*(nx+2)*YLAP*nz);
    ptr_t tmp_recv2= (ptr_t)malloc(sizeof(data_t)*(nx+2)*YLAP*nz);
    ...
    for(k=1;k<=nz;k++){
        for(j=1;j<=YLAP;j++){
            for(i=0;i<=nx+1;i++){
                j1=(k-1)*(nx+2)+i;
                //printf("i=%d, j=%d, k=%d, ny-YLAP+j=%d\n", i,j,k,ny-YLAP+j);
...
void exchange_boundary_z(ptr_t a0,const dist_grid_info_t *grid_info)
{
   ...
   ptr_t tmp_send1= (ptr_t)malloc(sizeof(data_t)*(nx+2)*(ny+2)*ZLAP);
   ptr_t tmp_send2= (ptr_t)malloc(sizeof(data_t)*(nx+2)*(ny+2)*ZLAP);
   ptr_t tmp_recv1= (ptr_t)malloc(sizeof(data_t)*(nx+2)*(ny+2)*ZLAP);
   ptr_t tmp_recv2= (ptr_t)malloc(sizeof(data_t)*(nx+2)*(ny+2)*ZLAP);


   int ldx = grid_info->local_size_x + 2 * grid_info->halo_size_x;
   int ldy = grid_info->local_size_y + 2 * grid_info->halo_size_y;
   int ldz = grid_info->local_size_z + 2 * grid_info->halo_size_z;
   nsize=(nx+2)*(ny+2)*ZLAP;
   for(k=1;k<=ZLAP;k++){
   for(j=0;j<=ny+1;j++){
    for(i=0;i<=nx+1;i++){
        k1=j*(nx+2)+i;
