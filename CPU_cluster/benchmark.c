...
int main(int argc, char **argv) {
    ...
    /*For simplicity, only allows cubic numbers of processes.*/
    float edge_slices = cbrt(info.p_num);
    if (!(edge_slices == ceil(edge_slices) && edge_slices == floor(edge_slices))) {
        printf("Number of processes must be a cubic number!");
    ...
    if(status == 0) {
        if(argc == 8) {
            ...
            printf("errors:\n    1-norm = %.16lf\n    2-norm = %.16lf\n  inf-norm = %.16lf\n time=%.16f\n",\
                    result.norm_1, result.norm_2, result.norm_inf,start-end);
